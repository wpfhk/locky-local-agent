...
def test_commit_dry_run(runner, workspace):
    with patch("actions.commit.run", return_value={"status": "ok", "message": "feat: x"}):
        result = runner.invoke(cli, ["commit", "--dry-run", "--workspace", str(workspace)])
    assert result.exit_code == 0


def test_commit_default(runner, workspace):
    with patch("actions.commit.run", return_value={"status": "ok", "message": "fix: y"}):
...
def test_format_cmd(runner, workspace):
    with patch("actions.format_code.run", return_value={
        "status": "ok", "language": "python",
        "black": {"status": "ok", "output": ""}, "isort": {"status": "ok", "output": ""},
    }):
...
def test_test_cmd_pass(runner, workspace):
    with patch("actions.test_runner.run", return_value={
        "status": "pass", "passed": 5, "failed": 0, "errors": 0, "duration": 1.0, "output": "5 passed",
    }):
...
def test_test_cmd_fail(runner, workspace):
    with patch("actions.test_runner.run", return_value={
        "status": "fail", "passed": 1, "failed": 2, "errors": 0, "duration": 0.5, "output": "1 passed, 2 failed",
    }):
        result = runner.invoke(cli, ["test", "--workspace", str(workspace)])
...
def test_test_cmd_verbose(runner, workspace):
    with patch("actions.test_runner.run", return_value={
        "status": "pass", "passed": 1, "failed": 0, "errors": 0, "duration": 0.1, "output": "",
    }) as mock:
...
def test_todo_cmd(runner, workspace):
    with patch("actions.todo_collector.run", return_value={"status": "ok", "total": 2, "items": [
...
def test_todo_cmd_no_items(runner, workspace):
    with patch("actions.todo_collector.run", return_value={"status": "ok", "total": 0, "items": []}):
        result = runner.invoke(cli, ["todo", "--workspace", str(workspace)])
    assert result.exit_code == 0
...
def test_scan_cmd_clean(runner, workspace):
    with patch("actions.security_scan.run", return_value={
        "status": "clean", "summary": {"critical": 0, "high": 0, "medium": 0, "low": 0}, "issues": [],
    }):
...
def test_scan_cmd_issues(runner, workspace):
    with patch("actions.security_scan.run", return_value={
        ...
        "issues": [{"severity": "critical", "file": "app.py", "line": 1,
                    "description": "hardcoded secret", "category": "hardcoded_secret"}],
    }):
        result = runner.invoke(cli, ["scan", "--workspace", str(workspace)])
    assert result.exit_code == 0
...
def test_scan_cmd_severity_filter(runner, workspace):
    with patch("actions.security_scan.run", return_value={
        "status": "clean", "summary": {}, "issues": [],
    }) as mock:
        runner.invoke(cli, ["scan", "--severity", "high", "--workspace", str(workspace)])
    _, kwargs = mock.call_args
...
def test_clean_dry_run(runner, workspace):
    with patch("actions.cleanup.run", return_value={
        "status": "ok", "removed": ["__pycache__"], "total_size_bytes": 100, "dry_run": True,
    }):
...
def test_clean_force(runner, workspace):
    with patch("actions.cleanup.run", return_value={
        "status": "ok", "removed": ["__pycache__"], "total_size_bytes": 100, "dry_run": False,
    }) as mock:
...
def test_deps_cmd(runner, workspace):
    with patch("actions.deps_check.run", return_value={
        "status": "ok", "dependencies": [], "outdated": [],
    }):
...
def test_env_cmd(runner, workspace):
    with patch("actions.env_template.run", return_value={
        "status": "ok", "output_file": ".env.example", "keys": ["DB_URL"],
    }):
...
def test_commands_registered():
    expected = {"commit", "format", "test", "todo", "scan", "clean", "deps", "env", "ask", "edit", "agent"}
