...
def test_ask_cmd_with_file(runner, workspace):
    with (
        patch("tools.ollama_guard.ensure_ollama", return_value=True),
        patch(
            "tools.ollama_client.OllamaClient.chat", return_value="정수"
        ) as mock_chat,
    ):
        result = runner.invoke(
            cli, ["ask", "x가 뭐야?", "code.py", "--workspace", str(workspace)]
        )
    assert result.exit_code == 0
...
def test_edit_cmd_file_not_found(runner, workspace):
    result = runner.invoke(
        cli, ["edit", "수정", "nope.py", "--workspace", str(workspace)]
    )
    assert result.exit_code == 0
...
def test_edit_cmd_dry_run(runner, workspace):
    ...
    with (
        ...
        result = runner.invoke(
            cli, ["edit", "x를 2로", "code.py", "--workspace", str(workspace)]
        )
    assert result.exit_code == 0
...
def test_agent_cmd_no_ollama(runner, workspace):
    with patch("tools.ollama_guard.ensure_ollama", return_value=False):
        result = runner.invoke(
            cli, ["agent", "포맷해줘", "--workspace", str(workspace)]
        )
    assert result.exit_code == 0
    # Ollama 없으면 error 반환
